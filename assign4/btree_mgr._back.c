...
//    BTreeNodePage *newRootNodePage = createNodePage(tree->mgmtData->bm->mgmtData->fh->totalNumPages, rKey->dt, tree->mgmtData->maxNumKeys);
    ...
    int oldIndex;
    newKeyPlaced = updateNewNode(tree, rKey, rid, nodePage, newNodePage, numKeysNewNode, newKeyPlaced, &oldIndex);
    ...
    /*        if((nodePage->bTreeNode->keys[oldIndex].v.intV > rKey->v.intV) || (newKeyPlaced)) {
            ...
            newRootNodePage->bTreeNode->keys[0].v.intV = newNodePage->bTreeNode->keys[0].v.intV;
            break;
        case DT_FLOAT:
        ...
        case DT_STRING:
            if((strcmp(nodePage->bTreeNode->keys[oldIndex].v.stringV, rKey->v.stringV) > 0) || (newKeyPlaced)) {
                newRootNodePage->bTreeNode->keys[0].v.stringV =
                                 (char *) malloc(strlen(nodePage->bTreeNode->keys[oldIndex].v.stringV) + 1);
                strcpy(newRootNodePage->bTreeNode->keys[0].v.stringV, nodePage->bTreeNode->keys[oldIndex].v.stringV);
                oldIndex--;
            } else {
     ...
     */
    newKeyPlaced = updateOldNode(tree, rKey, rid, nodePage, numKeysOldNode, newKeyPlaced, oldIndex);
...
//    BTreeNodePage *newNodePage = createNodePage(tree->mgmtData->bm->mgmtData->fh->totalNumPages, key->dt, tree->mgmtData->maxNumKeys);
    ...
    *returnNewPageNum = newNodePage->pageNum;
    ...
    bool newKeyPlaced = false;
    int oldIndex;
    newKeyPlaced = updateNewNode(tree, key, rid, nodePage, newNodePage, numKeysNewNode, newKeyPlaced, &oldIndex);
    ...
    /* Now let's update the old node.
       ...
       any action. */
    newKeyPlaced = updateOldNode(tree, key, rid, nodePage, numKeysOldNode, newKeyPlaced, oldIndex);
